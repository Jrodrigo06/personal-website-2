...
def compute_embeddings(
    paths: list[Path], cache: dict[str, np.ndarray], force: bool
) -> tuple[np.ndarray, int, int]:
    """Return (matrix [n,512], n_embedded, n_cached). Prunes stale cache keys."""
    keys = [cache_key(p) for p in paths]
    needed = force or any(k not in cache for k in keys)
    ...
    return np.vstack(vectors), n_embedded, n_cached
...
def run_umap(embeddings: np.ndarray, seed: int) -> np.ndarray:
    ...
    if n < 3:
        ...
        return np.column_stack(
            [np.linspace(0.0, 1.0, n), np.full(n, 0.5)]
        ).astype(np.float32)
...
def write_outputs(manifest: list[dict], json_path: Path, ts_path: Path) -> None:
    json_path.parent.mkdir(parents=True, exist_ok=True)
    json_path.write_text(json.dumps(manifest, indent=2) + "\n")

    ts_path.parent.mkdir(parents=True, exist_ok=True)
    entries = json.dumps(manifest, indent=2)
    ts = (
        ...
        "  height: number;\n"
        "}\n\n"
        f"export const photos: PhotoNode[] = {entries};\n"
    )
...
def main() -> None:
    ...
    embeddings, n_embedded, n_cached = compute_embeddings(paths, cache, args.force)
    ...
    for path in paths:
        base = output_basename(path)
        make_image(
            path, args.thumbs_dir, base, args.thumb_width, args.thumb_quality, args.force
        )
        wh = make_image(
            path,
            args.display_dir,
